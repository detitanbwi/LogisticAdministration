    ...
    protected function formatInvoiceRows($inv, &$state)
    {
        ...
        for ($i = 0; $i < $rowCount; $i++) {
            ...
            $row = [
                ...
                $i == 0 ? $state['row_index'] : '',
                $i == 0 ? $inv->no_invoice : '',
                $i == 0 ? ($inv->tgl_masuk ? $inv->tgl_masuk->translatedFormat('d-M-Y') : '-') : '',
                ...
                $i == 0 ? ($inv->container && $inv->container->etd ? \Carbon\Carbon::parse($inv->container->etd)->translatedFormat('d-M-Y') : '-') : '',
                $i == 0 ? ($inv->container && $inv->container->eta ? \Carbon\Carbon::parse($inv->container->eta)->translatedFormat('d-M-Y') : '-') : '',
    ...
    protected function formatFinanceRows($fin, &$state)
    {
        ...
        for ($i = 0; $i < $rowCount; $i++) {
            ...
            $row = [
                ...
                $i == 0 ? ($inv->container && $inv->container->etd ? $inv->container->etd->translatedFormat('d-M-Y') : '-') : '',
                $i == 0 ? ($inv->container->asal->nama_tujuan ?? '-') : '',
                $i == 0 ? ($inv->container->tujuan->nama_tujuan ?? '-') : '',
                $i == 0 ? ($inv->container && $inv->container->eta ? $inv->container->eta->translatedFormat('d-M-Y') : '-') : '',
                ...
                $i == 0 ? ($inv->terima_barang ? $inv->terima_barang->translatedFormat('d-M-Y') : '-') : '',
                $i == 0 ? strtoupper($fin->status_tagihan ?? '-') : '',
                $i == 0 ? strtoupper($inv->layanan->nama ?? '-') : '',
                $i == 0 ? ($fin->ditagih_ke ?? '-') : '',
                $i == 0 ? ($fin->tanggal_tagih ? \Carbon\Carbon::parse($fin->tanggal_tagih)->translatedFormat('d-M-Y') : '-') : '',
                $i == 0 ? $this->calculateTunggakan($fin) : '',
                $i == 0 ? strtoupper($inv->status_pembayaran ?? '-') : '',
                $i == 0 ? mb_strtoupper($inv->pkp_status ?? '-') : '',
                $i == 0 ? ($fin->tgl_transfer ? \Carbon\Carbon::parse($fin->tgl_transfer)->translatedFormat('d-M-Y') : '-') : '',
                $i == 0 ? ($inv->catatan_muntahan ?? '-') : '',
                $i == 0 ? ($fin->catatan ?? '-') : '',
            ];
            $rows[] = $row;
